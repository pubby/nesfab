...
[NOTE]
Just because the compiler accepts a `sram` does not mean that corresponding hardware exists in the real world.
...
[NOTE]
BNROM is usually the better mapper for NESFab.
...
The `macro` keyword generates and compiles a new source file by substituting its arguments into a `.macrofab` file.
...
- `+static`: Allocate the function or data in every bank. 
...
- `palette_25`: Converts 32-byte palettes into 25-byte palette.
