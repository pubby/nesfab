...
- `employs`: A list of `vars` groups. See <<kw_employs_mod>>.
...
Assigning to a global variable in a `vars` block sets its default value.
...
----
vars /my_group
    U score = 0 // Set a default value for 'score'
    UU player_x
    UU player_y
