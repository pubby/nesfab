...
It is a work in progress, so comments and contributions are welcome.
...
This option prints an estimate of ROM usage after compiling. 
...
These options insert `inc $4011` writes into the program.
Such writes are used by certain mappers, such as <<mapper_rainbow>>, to implement expansion sound on the DMC channel.

- `--ipcm` enables IPCM write insertion.
- `--ipcm-period` determines the approximate number of cycles between each write.

By default, the value of `ipcm-period` is 50.

*Command-line usage:*
----
nesfab --ipcm
nesfab --ipcm-period 50
----
...
[NOTE]
The writes are added liberally, without perfect accuracy. Large periods have especially poor accuracy.
...
The `byte_block` of `label` primarly exists to provide a scope.
Without modifiers, there is no difference between the syntax above, and this:
...
=== `subalign`, `subsegment` [[kw_subalign_subsegment]]

See <<mod_subalign_subsegment>>.
...
=== Field Modifier Flags [[mod_flags_fields]]
...
=== `subalign`, `subsegment` [[mod_subalign_subsegment]]

The `subalign` and `subsegment` <<modifiers, modifiers>> modifiers force <<kw_label_asm, byte block labels>> to an alignment relative to their scope.

- `subalign` forces the label to be aligned.
- `subsegment` forces the data scoped inside the label to not cross any alingment boundaries.
...
- `value` is an expression of type Int.
...
In the example above, 7 padding bytes would be inserted after `U(10)` to align `label a`.

[NOTE]
These modifiers do not affect the alignment of the containing allocation. 
Use <<mod_flags, `+align`>> for that.
...
- `offset` is an optional expression of type `U`, used as the offset. If absent, `0` is used.
...
[NOTE]
For <<type_index, index types>>, `value` is added to `offset` to index into the type's array.
...
- `value` is an expression with a <<type_tea, TEA type>>, <<type_vec, VEC type>>, or a pointer to a <<type_paa, PAA type>>.
- `offset` is an expression of type `UU`, used as the offset. If absent, `0` is used.
...
`len` expressions return the number of array elements, or the final count for the <<iota>> keyword.
...
==== `len iota` [[len_iota_expr]]

Returns the final count of an <<iota>> value.

Syntax:
----
len iota.identifier
----
- `identifier` is the name of the iota value.

Example:
----
ct Int expr_len = len iota.foo
----

=== `iota` [[abs_expr]]

The `iota` keyword implements parse-time counters. 
Each time an `iota` expression is parsed, the compiler replaces it with an expression of type `Int` equal to the current count.
These counters always start at 0, and always increment by 1.

Syntax:
----
iota.identifier
----
- `identifier` is the name of the counter.

Example:
----
UU first = iota.foo  // Value of 0
UU second = iota.bar // Value of 1
----

[NOTE]
See <<len_iota_expr, `len`>> for a way to get the final count.
...
Index types are a special variant of unsigned integers. 
Their values represent indexes into a single global array.
...
II.array
----
- `array` is the name of the global array.

Index types behave similarly to pointers, but their values are indexes, not addresses.
Like pointers, the <<array_access_u, `[]` operator>> can be used to access the pointed-to value.

Example:
----
I.foo my_index = 10 // Create an index into array foo at position 10.
my_index[0] = 20    // Sets my_index[10+0] to 20.
my_index += 1       // Increment the index to position 11.
my_index[0] = 30    // Sets my_index[11+0] to 30.
my_index[2] = 40    // Sets my_index[11+2] to 40.
----

Index types are not required to access arrays; `U` and `UU` can be used instead.
However, index types offer a few advantages:

1. They reduce typing. Instead of typing `foo[x]`, you can type `x[]`.
2. They better decribe the purpose of variables.
3. They provide better type safety.
...
Arithmetic types are defined as all <<type_integer, integer types>>, <<type_fixed, fixed-point types>>, <<type_index, index types>> and <<type_bool, `Bool`>>.

=== Quantity Types [[type_quantity]]

Quantity types are types which can represent quantities.
They are defined as all <<type_integer, integer types>> and <<type_fixed, fixed-point types>>.
