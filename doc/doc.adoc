...
you'll want to write tools and conversion scripts in other languages - like Python or C.

=== Installation Guide

To get started with NESFab, download it from TODO and install it. 
You will also want a NES emulator, like https://fceux.com/[FCEUX] or https://fceux.com/[Mesen].

The `nesfab` tool compiles `.fab` source code files into `.nes` ROMs. 
It can be used with the command-line, or by clicking and dragging a source code or configuration file onto the executable.

Let's test that now.  Create a file called `hello.fab` and save it with code below.

----
// TODO: hello world
----

Now let's compile it. If using the command-line, run:

`nesfab hello.fab`

Otherwise, drag the `hello.fab` file onto the `nesfab` executable.

When done, the compiler should have produced a `a.nes` file in the same directory,
which is the default name of all compiled binaries. 
Try running `a.nes` in your NES emulator.
You should see the screen flash with colors.

=== Options and Configuration Files

The `nesfab` compiler accepts options both on the command-line, and via configuration files.
For most projects, a single configuration file is the ideal, so this tutorial will focus on that.

Below is an example configuration file: `config.cfg`. The `output` option determines the name of the `.nes` file, while `input` lists a single source file.
----
output = "game.nes"
input = "hello.fab"
----
...
Otherwise, drag the `config.cfg` file onto the `nesfab` executable.

It should produce the same result as before, but save the binary as `game.nes` instead of `a.nes`,
as the `output` option was set.
...
If either operand is signed, the return type is signed too, but otherwise it is unsigned.
...
[NOTE]
Multiplying two variables together is a very slow operation, as the NES lacks a hardware multiplier,
but multiplying a variable by a constant is faster, as the compiler can convert the expression to a sequence of shifts and adds.
However, if you need to do lots of multiplications, consider using lookup tables instead.

==== Assign by Multiply (`*=`) [[assign_multiply]]

Multiplies its operands together, then assigns the value to the lvalue left operand. 
...
which are sets of characters along with a mapping from each character to byte values.
...
----
charmap identifier_name("string", 's')
----

- *Optional:* `identifier_name` is the name of the charmap. 
  When left out, the default `charmap` is defined.
- *Mandatory:* `"string"` is a string literal, defining the characters of the charmap. 
...
- *Optional:* `'s'` is a charcter literal, defining the sentinel. 
  When left out, no sentinel is defined.
...
`charmap` accepts a single group in its `data` <<modifiers, modifiers>>.
If defined, string literals using the `charmap` become valid operands to <<at, operator `@`>> and <<amper, operator `&`>>.
