...
- For <<type_tea, TEAs>>, if the element type is a <<type_struct, structure>>, the first member is insterted first as an array, then the second, and so on. Otherwise, the first element is inserted first, then the second, and so on.
