...
- <<mappers, Mapper> banks are handled automatically.
- Multi-byte and fixed-point arithmetic is well supported.
...
- Although NESFab performance is good, writing assembly by hand can surpass it. 
  NESFab does support mixing assembly and procederal code together, but there are limitations to doing so. 
- NESFab is not as tried-and-true as the alternatives. They may be bugs and missing features.

== Quirks and Surprises

If you're coming from another language, you might be surprised by a few of NESFab's quirks. 
The most flagrant ones are listed before.

=== Types and Values

- Everything is a value type. Nothing is passed by reference.
...
- Global variables and data are partitioned into used-defined <<groups, groups>>.  
...
NESFab is available at https://github.com/pubby/nesfab[Github]. 
...
It is only usable inside of a <<todo, byte-block>>.
...
A 24-bit pointer is simply a 16-bit address paired with an 8-bit integer representing the bank.
