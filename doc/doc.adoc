...
In many programming languages, operators for bitwise math have a very low precedence.
This tradition dates back to the https://en.wikipedia.org/wiki/B_(programming_language)[B programming language],
where bitwise operators were used in place of logical operators (like `&&` and `||`).
NESFab bucks this tradition and uses a <<binop, higher precedence for these operators>>.

This means that code like: `A & B == C` is parsed as `(A & B) == C`, instead of `A & (B == C)`.

==== Value of assignment operators
...
For most projects, a single configuration file is the ideal, so this section will focus on that.
