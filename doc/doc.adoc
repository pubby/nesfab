...
As it decompresses into chunks of 8 bytes, it can't work with arbitrary data.
...
The `data` <<modifiers, modifier>> is used to document which `data` <<TODO group, groups>> a function uses.
...
The `vars` <<modifiers, modifier>> is used to document which `vars` <<TODO group, groups>> a function uses.
...
and has the same representation as <<type_ptr, pointers>>, ignoring <<TODO, groups>>.
