...
`identifier` refers to the name of a label in the enclosing function.
...
In the process, global variables will be reset to their default value,
...
The `return` statement ends the execution of the enclosing function,
...
Instead, it provides access to the enclosing function's return value.
...
In general, if `ready` is `true`, all global variables are in a stable, thread-safe state.
...
[NOTE]
There is more than one way to achieve thread safety. See <<kw_fence>>, for example.
