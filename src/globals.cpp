...
#define RUN_O(o, ...) do { if(o(__VA_ARGS__)) { \
    ...
    ir.assert_valid(); \
        ...
        do
        {
            ...
            RUN_O(o_bool_tables, log, ir);
            if(post_byteified)
                RUN_O(o_switch_tables, log, ir);
            save_graph(ir, fmt("E_post_tables_%_%", post_byteified, iter).c_str());

            RUN_O(o_chain, log, ir);
            save_graph(ir, fmt("F_post_chain_%_%", post_byteified, iter).c_str());

            // 'o_loop' populates 'ai_prep', which feeds into 'o_abstract_interpret'.
            // Thus, they must occur sequentially.
            reset_ai_prep();
            save_graph(ir, fmt("G_pre_loop_%_%", post_byteified, iter).c_str());
            RUN_O(o_loop, log, ir, post_byteified, sloppy());
            save_graph(ir, fmt("H_pre_ai_%_%", post_byteified, iter).c_str());
            RUN_O(o_abstract_interpret, log, ir, post_byteified);
            save_graph(ir, fmt("I_post_ai_%_%", post_byteified, iter).c_str());

            RUN_O(o_remove_unused_ssa, log, ir);

            save_graph(ir, fmt("J_pre_motion_%_%", post_byteified, iter).c_str());
            RUN_O(o_motion, log, ir);
            save_graph(ir, fmt("K_post_motion_%_%", post_byteified, iter).c_str());
    ...
    {
        bool modified = false;
        modified |= o_shl_tables(log, ir);
        modified |= switch_partial_to_full(ir);
        modified |= o_switch_tables(log, ir);
        if(modified)
            optimize_suite(false);
    }
